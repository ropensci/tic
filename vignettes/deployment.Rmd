...
`usethis::browse_github_pat()` helps etting one up if you haven't done so already.
...
- Calls `step_setup_ssh()` if the environment variable `"BUILD_PKGDOWN"` is set in the Travis CI build. 
  ...
  Argument `name = private_key_name` can be ignored as long as no custom private key name was used during `travis::use_travis_deploy()`. 
