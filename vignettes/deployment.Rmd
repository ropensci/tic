...
*rci* handles the deployment as follows:
...
2. A private key is encoded in base64 and stored in an environment variable on Travis. During the CI run, _rci_ installs it in the `~/.ssh` directory so that it can be used for authenrciation:
...
_rci_ has the ability to automate the update tasks and having the site reflecting the latest state of the package.

The following example shows how _rci_ deploys a `pkgdown` site on Travis.
...
1. Travis executes `rci::before_deploy()` which will search for instructions regarding the `before_deploy()` stage in `rci.R`. 
...
3. This "script" is calling `rci::deploy()`.

4. All this is happening on every branch. If you want to restrict it to certain branches only, you need to set a condition in `rci.R`.

    What happens now is that `rci::deploy()` again searches in `rci.R` for the "deploy" stage and then runs whatever is specified there.

    With the default template of `rci.R`, the following will be executed:
...
To use this option with *rci*, specify `branch = "gh-pages", path = "docs"` in `step_push_deploy()`.
...
There are two ways to restrict the deployment with *rci* and environment variables: 

1. Setting the condition in `rci.R` (recommended)
...
_rci_ can make use of implicit set environment variables of the build and additional ones added by the user.
  ...
  include:
    - stage: stage1
      ...
      before_install: 
        ...
        - R -q -e 'remotes::install_github("ropenscilabs/rci")'
        - R -q -e 'rci::prepare_all_stages(); rci::before_install()'
      [...]
      deploy:
        provider: script
        script: R -q -e 'rci::deploy()'
...
An example project that uses the latter strategy is [mlr](https://github.com/mlr-org/mlrng/blob/master/rci.R).
