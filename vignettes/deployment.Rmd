...
1. When calling `use_travis_deploy()` (either directly or indirectly by using `use_tic()`), a public key is added to GitHub repository under the "Deploy keys" section and the user receives an e-mail:
...
Without a _pkgdown_ site, these package resources need either be collected from various places (GitHub repository, CRAN function reference) or do even require a local installation of the package if vignettes are placed in custom locations.
...
1. Travis executes `tic::before_deploy()` which will search for instructions regarding the `before_deploy()` stage in `tic.R`. 
    ...
    This block calls the function `step_setup_ssh()` if the environment variable `"BUILD_PKGDOWN"` is set in the Travis build.
    That is just an example of an option that can be set to run certain commands (or stages) conditionally (see [secion "Running stages conditionally" in the "Advanced" article](advanced.html#running-stages-conditionally) for more examples).
