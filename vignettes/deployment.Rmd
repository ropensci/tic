...
_tic_ uses the [CI client packages](ci-client-packages.html) {travis} and {circle} for setting up deployment on the CI systems.
...
When calling `circle::use{circle}deploy()` directly (or indirectly via `tic::use_tic()`), a so called "github-user-key" is created and stored in the Circle CI repo.
...
- Call `step_setup_ssh()` if the environment variable `"BUILD_PKGDOWN"` is set in the Travis build. This step sets up SSH key previously added to the Github and Travis via `travis::use_travis_deploy()`.
