...
## Special case: `custom` and `custom-deploy` templates

If you are using the `custom` or `custom-deploy` deploy template (e.g. via `tic::use_ghactions_yml("custom")`), `tic::update_yml()` will ignore the header part until the `env:` key completely.
This enables you to can change the build trigger settings like `push:`, `pull_request:` without having to worry that these get be overwritten during template updates.
This also includes the runner configs.
Hence, these template types can be used for more specialized GHA setups which can still profit from automatic updates to its core blocks and their respective action versions.

Note that the feature is in beta state and issues are expected, especially for edge cases.
...
Just put this workflow next to `.github/workflows/tic.yml` and it will silently do its job.
