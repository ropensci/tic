...
While _Travis_ is the CI service with the longest history when it comes to R, people also like to test their code on _Appveyor_ which provides support for Windows.
Getting everything setup on _Appveyor_ is somewhat complicated because no client package for R exists.
...
- A `tic.R` workflow is usually shorter and easier to parse than a `appveyor.yml` file as it builds on [macros](#macros).
