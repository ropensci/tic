...
The easiest way to use *rci* for CI services is to call `usethis::use_ci()`.
This will create templates for both "Travis CI" and "Appveyor" and initialize all the required authenrciation steps for deployment.
...
First, authenrciation with Travis (a browser window opens) and GitHub is initiated (another browser window opens).
Also, the repo is actived on Travis.

```
usethis::use_ci()
✔ Setting active project to '/home/pjs/git/rci.package'
✔ GitHub is already initialized
Authenrciating with GitHub
Waiting for authenrciation in browser...
Press Esc/Ctrl + C to abort
Authenrciation complete.
Authenrciating with Travis
Authenrciating with GitHub
Waiting for authenrciation in browser...
Press Esc/Ctrl + C to abort
Authenrciation complete.
Finished activating repo pat-s/rci.package on Travis CI.
```

Next, you will be asked if the current `.travis.yml` should be replaced with the template from *rci* (if one exists).
...
Selection: 1
Added rci.R from template.
✔ Adding '^rci\\.R$' to '.Rbuildignore'
```
...
Press Esc/Ctrl + C to abort
Authenrciation complete.
Finished adding deploy keys on GitHub for repo pat-s/rci.package.
Successfully added public deploy key 'travis+rci for pat-s/rci.package' to GitHub for 
pat-s/rci.package. You should receive a confirmation e-mail from GitHub. Delete the 
...
```
Finished adding private environment variable id_rsa to pat-s/rci.package on Travis CI.
Successfully added private deploy key to pat-s/rci.package as secure environment variable
id_rsa to Travis CI.
```
...
The suggested description 'travis+rci for pat-s/rci.package' has been copied to the 
...
Finished adding private environment variable GITHUB_PAT to pat-s/rci.package on Travis CI.
```
...
However, the heavy lifting in the background is actually done by the R packages *travis* and *rci* . 
See [here](rci-usethis-travis.html) for more detailed information on how *rci* , *travis* and *usethis* work together.

# Explanation of the basic template

After having called `usethis::use_ci()` you will find a `.travis.yml`, `appveyor.yml` and a `rci.R` file in your repo.
Usually you do not need to touch `appveyor.yml` and `.travis.yml` anymore.
All build customizations are done in `rci.R` and apply to both services.
If you want more information about the whole build lifecycle, check the [Build lifecycle](build-lifecycle.html) vignette.

The basic `rci.R` template looks as follows:
...
The env variable is set in the [`.travis.yml`](https://github.com/ropenscilabs/rci/blob/d3c9ffab7e42ef4bf0a3113337c42dbaf592146c/.travis.yml#L45-L47) template, but only for the job that runs on the current R-release version:
...
*rci* can be used for various R projects:

- [rci.package](https://github.com/ropenscilabs/rci.package): R packages with [_pkgdown_](https://pkgdown.r-lib.org/) documentation
- [rci.packagedocs](https://github.com/ropenscilabs/rci.packagedocs): R packages with [_packagedocs_](http://hafen.github.io/packagedocs/) documentation
- [rci.drat](https://github.com/ropenscilabs/rci.drat): CRAN-like package repositories with [_drat_](http://dirk.eddelbuettel.com/code/drat.html)
- [rci.blogdown](https://github.com/ropenscilabs/rci.blogdown): Blogs with [_blogdown_](https://bookdown.org/yihui/blogdown/)
- [rci.bookdown](https://github.com/ropenscilabs/rci.bookdown): Books with [_bookdown_](https://bookdown.org/)
- [rci.website](https://github.com/ropenscilabs/rci.website): Websites with [_rmarkdown_](https://rmarkdown.rstudio.com/)
- [rci.figshare](https://github.com/ropenscilabs/rci.figshare): Deploying artifacts to [figshare](https://figshare.com/) (work in progress).
- [rci.covrpage](https://github.com/ropenscilabs/rci.covrpage): Unit test summary report.
...
Under the hood, it uses the framework [HUGO](https://gohugo.io/) which gets installed by the respective `rci.R` [template](https://github.com/krlmlr/rci.blogdown/blob/975aedd43fec1dd55e8348eccfca2c7c5f663006/rci.R#L5) in the "install" section:
...
*rci* is smart enough to detect your project automatically when calling `usethis::use_ci()` and will add the correct template.

**Note:** Currently, publishing to https://figshare.com/ doesn't work.
Also, publishing to https://zenodo.org/ is work in progress.

## *rci* based R projects from the community

The templates we provide with *rci* are minimal working examples.
In the following we list various R projects with non-standard/tailored *rci*-based workflows.
...
The advanced usage of *rci* is described in more detail in the [Advanced Usage](advanced.html) arrcile:
...
- [Troubleshooting: Running *rci* locally](advanced.html#troubleshooting-running-rci-locally)
- [Troubleshooting: Enter into the Travis build](advanced.html#troubleshooting-running-rci-locally)
- [Writing custom steps](custom-steps.html)

The build lifecycle when using *rci* is explained in the [Build lifecycle](build-lifecycle.html) arrcile.
