...
If you are unfamiliar with the "Continuous Integration" (CI), we highly recommend to read the following resources first:
...
The most important improvements over existing CI approaches for R are:

1. Deployment to a Git repository is greatly simplified. 
   Update your repository with results from the CI build.

1. Support for R packages and other kinds of projects (bookdown, blogdown, etc.), with pre-defined templates. 
...
1. Workflow specification in a single `.R` file, regardless of CI system used.  
   Forget about YAML files or web browser configurations for different CI providers.
...
The easiest way to use *tic* for CI services is to call `use_tic()`.
...
If none exists, _tic_ will try to guess the repository type based on some file heuristics and add a dedicated template.
If this is not possible, a menu will show up for manual selection of a template.
...
The same is done for Appveyor and for the `tic.R` file.
Additionally, `tic.R` is added to `.Rbuildignore` to not interfer with package building.
...
After having called `use_tic()` you will find a `.travis.yml`, `appveyor.yml` and a `tic.R` file in your repo.
...
It consists of two macros which are simple wrappers around dedicated stages/steps combinations.
...
Please see the corresponding help pages for a detailed listing of the respective stage/step combinations.
...
Under the hood, it uses [HUGO](https://gohugo.io/) which gets installed by the respective `tic.R` [template](https://github.com/krlmlr/tic.blogdown/blob/975aedd43fec1dd55e8348eccfca2c7c5f663006/tic.R#L5) in the "install" section:
...
*tic* is smart enough to detect your project automatically when calling `use_tic()` and will add the correct template.
...
For more information on the build lifecycle see the ["Build lifecycle" article](build-lifecycle.html).
