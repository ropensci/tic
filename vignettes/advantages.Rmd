---
title: "The advantages of rci"
author: "Patrick Schratz, Kirill Müller"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The advantages of rci}
...
So, what's the advantage of using *rci* over the standard CI process?
...
- an automatically generated [test summary page](https://github.com/yonicd/covrpage) of a package 

When the goal is to use CI services for deployment, some preparatory work needs to be done.
*rci* takes care of all of this (setting up SSH keys, granting permission for the repo on GitHub and the CI services, creation of a personal Personal Access Token (PAT)).
When using `travis::use_ci()` the whole preparation process is streamlined and only needs to be called once for every repo.

For more information on automatic deployment in *rci*, take a look [here](deployment.html).

## Support for various R projects

*rci* was developed with the aim to not only simplify R package development using CI services but also to support other R project types (bookdown, blogdown, drat, website and figshare deployment). 

Each of these projects has its own standardized structure. 
*rci* detects this structure and adds templates tailored towards this specific project type to the repository when calling `usethis::use_ci()`.
See [here](rci.html#example-projects) for a list of supported project types including links to minimal example repositories. 

## CI-Agnosrci workflows

What does "CI-Agnosrci" mean and why do we need it?
...
We could list way more differences - but that's exactly the point when *rci* comes in!

- Rather than dealing with all the CI differences, *rci* enables the specification of the complete workflow in an external R script file `rci.R`.
- The commands specified in there will work the same way on every CI service that is supported by *rci*.
...
- A `rci.R` workflow is usually shorter and easier to parse than a `.travis.yml` or `appveyor.yml` file.
...
All of the commands and arguments specified there can also be set in `rci.R` using R syntax.
...
*rci* instead uses  [*rcmdcheck*](https://github.com/r-lib/rcmdcheck), a wrapper around `R CMD build` and `R CMD check`.
...
```{r eval = FALSE}
R CMD check ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
─  using log directory ‘/tmp/RtmpM4zdgT/file12d469173dd1/rci.Rcheck’ (581ms)
─  using R version 3.5.1 (2018-07-02)
─  using platform: x86_64-pc-linux-gnu (64-bit)
─  using session charset: UTF-8
─  using options ‘--no-manual --as-cran’
✔  checking for file ‘rci/DESCRIPTION’
─  checking extension type ... Package
─  this is package ‘rci’ version ‘0.2.13.9010’
...
E  checking package dependencies (806ms)
   ...
   See
     ‘/tmp/RtmpM4zdgT/file12d469173dd1/rci.Rcheck/00check.log’
   for details.
   ...
   manual.
── R CMD check results ───────────────────────────────────────────────────────────────────────────────────────────────────────────── rci 0.2.13.9010 ────
Duration: 1m 1.3s
...
When using *rci*, all dependencies (the ones of the package and also the ones of all stages of the CI build) are installed in the "before_install" stage. 
...
*rci* comes with the ability to [emulate a CI run locally](advanced.html#emulate-a-ci-run-locally) and [debug problems in the config file](advanced#troubleshooting-running-rci-locally) `rci.R` by calling `load_from_file()` locally.
