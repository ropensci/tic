...
Steps and stages can be run conditionally using the control workflow of _rci_.
...
# Troubleshooting: Running rci locally

*rci* can be emulated locally in two ways:

## Check for syntax errors

You can run check on possible syntax errors in `rci.R` using `load_from_file()`.
This function will just source `rci.R` to check for possible misspecification errors.
...
✔  creating vignettes (6.6s)
   Warning: ‘inst/doc’ file
       ‘rci-usethis-travis.R’
     ignored as vignettes have been rebuilt.
     Run R CMD build with --no-build-vignettes to prevent rebuilding.
─  checking for LF line-endings in source and make files and shell scripts
─  checking for empty or unneeded directories
─  building ‘rci_0.2.13.9010.tar.gz’
   
── R CMD check ──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────
─  using log directory ‘/tmp/RtmpM4zdgT/file12d42bb00e56/rci.Rcheck’ (1.2s)
...
- `SystemRequirements`: *rci* is not yet capable of automatically determining system requirements specified in DESCRIPTION files of an R package. 
