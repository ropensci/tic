...
It has little benefit to run this on multiple R versions as the outcome will usually be the same. 
...
When you use multiple R versions, Travis/Appveyor automatically set up a build matrix with each build having [set an environment variable for the R version](https://docs.travis-ci.com/user/languages/r/#Environment-Variables) used.
...
Here, we set `warnings_are_errors = TRUE` for `getRversion() >= "3.2")`.
Of course this only has an effect if testing on `getRversion() >= "3.2")` is actually specified in `.travis.yml` (see [here](https://github.com/ropenscilabs/tic/blob/36a1b88797e8eb7218cfb767b65490452d47ad8e/.travis.yml#L39-L45) for an example).
...
All this conditioning is specified in `tic.R`, which is a plain R file that is processed internally with `source()`.
...
One could be the requirement of the _magick_ package for a successfull favicon creation for a `pkgdown` website.
...
We introduced how to run *tic* locally when discussing the [advantages of using tic](advantages.html#easier-troubleshooting).
...
You can use some shortcuts to act in the VM:
