...
See [Troubleshooting: Running tic locally](advanced#troubleshooting-running-tic-locally) for more info about this point.
...
In this case it searches for `get_stage("deploy")` in particular.
If this call was found, all subsequent steps declared in this stage will be run.
...
Usually, steps are appended to stages by a pipe operator:
...
You can test this yourself by calling `tic::deploy()` locally if you have a `tic.R` file in your repo.
...
See [here](advantages.html) for details.
...
Macros in `tic` are combinations of steps for reoccuring workflows.

We already [mentioned](build-lifecycle.html#details-of-stages) `add_package_checks()` which is essentially a macro for testing R packages that add steps across several stages.
