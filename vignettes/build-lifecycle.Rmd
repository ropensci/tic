...
The `after_xxx` stages are run unconditionally, dependent on their super-class name.
For example stage `after_success` will only be run if the "script" stage executed successfully, i.e. without error.
The `after_deploy` stage will only be run if there the `deploy` stage was run before.

*tic* also uses the stages approach and forwards all action specifications that should be run in a stage to the `tic.R` file.
The steps can then be specified in an CI-agnostic way using R syntax.
Parts of the `.travis.yml` file are not meant to be edited anymore (at least not for the specification of the stages).
...
For testing purposes, all stages and steps defined in `tic.R` can be executed by calling `tic()`.
...
*tic* uses the [pipe operator](https://magrittr.tidyverse.org/) to and the `add_step()` function to chain steps in `tic.R`, for example
...
1. `step_rcmdcheck()` in the "script" stage, using the `warnings_are_errors`, `notes_are_errors`, `args` and `build_args` arguments.
