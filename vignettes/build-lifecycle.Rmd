...
*rci* also employs an approach based on stages.
All action specifications that should be run in each stage are defined in the `rci.R` file.
The steps are specified in an CI-agnosrci way using R syntax.
A large part of the `.travis.yml` file consists of glue code and is not meant to be edited anymore.


```yml
# DO NOT CHANGE THE CODE BELOW
before_install: R -q -e 'install.packages(c("remotes", "curl", "rci")); rci::prepare_all_stages(); rci::before_install()'
install: R -q -e 'rci::install()'
after_install: R -q -e 'rci::after_install()'
before_script: R -q -e 'rci::before_script()'
script: R -q -e 'rci::script()'
after_success: R -q -e 'rci::after_success()'
after_failure: R -q -e 'rci::after_failure()'
before_deploy: R -q -e 'rci::before_deploy()'
deploy:
  provider: script
  script: R -q -e 'rci::deploy()'
  on:
    all_branches: true
after_deploy: R -q -e 'rci::after_deploy()'
after_script: R -q -e 'rci::after_script()'
# DO NOT CHANGE THE CODE ABOVE
```

In a nutshell, the workflow is as follows:

`.travis.yml`/`appveyor.yml` -> `rci.R` -> commands/step to execute

An important point to note: The R code declared in `rci.R` is not meant to be run manually.
It also does not trigger a CI build.
All commands just define the workflow of the CI build.
The workflow can be loaded using `load_from_file()`, this will not run any of the commands defined there.
For testing purposes, all stages and steps defined in `rci.R` can be  executed by calling `run_all_stages()`.
This latter emulates a CI build on your local system.
See [Troubleshooting: Running rci locally](advanced#troubleshooting-running-rci-locally) for more information.

### Accessing a single stage

The steps which are executed in each stage are specified in `rci.R`.
A stage is executed by calling the respective *rci* function; for example for stage "deploy" `rci::deploy()`.
This function then sources `rci.R` and collects all steps added to `get_stage(<stage name>)`, e.g. `get_stage("deploy")` for the "deploy" stage".
Again, remember that the order of the stages is fixed (see [here](#stages)), it does not matter in which order you declare the stages in `rci.R`.

### Details of stages

#### before_install & install

An important stage for *rci* is the `before_install` stage.
Here, *rci* itself gets installed and runs `prepare_all_stages()`.
This function ensures that all subsequent steps can be executed.
Essentially it calls the `prepare()` function of all steps that were declared in `rci.R`.
...
*rci* uses the [pipe operator](https://magrittr.tidyverse.org/) and the `add_step()` function to chain steps in `rci.R`, for example
...
We grouped the steps defined by the *rci* package in the following.
Please see the respective help pages of the functions for more detailed documentation.

#### Basic

- `step_hello_world()`: print "Hello, World!" to the console, helps testing a rci setup
...
Macros in *rci* are combinations of steps for reoccurring workflows to keep `rci.R` tidy.
...
If you manually add these steps to stages in `rci.R` while `do_package_checks()` is still present, the steps will be executed twice. 
...
If you have a good use case for a macro, let us know by opening an [issue](https://github.com/ropenscilabs/rci/issues).
