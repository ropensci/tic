...
load_from_file_ <- function(path = "rci.R", ..., mtime = file.mtime(path)) {
...
#'   Path to the stage definition file, default: `"rci.R"`.
#' @param ... ignored
...
#' rci's domain-specific language
#'
...
#'   An object of class [RciStep], usually created by functions
...
add_step <- function(stage, step) {
  ...
  tryCatch(
    step <- force(step),
    error = function(e) {
      stop("Error evaluating the step argument of add_step(), expected an object of class RciStep.\n",
           "Original error: ", conditionMessage(e), call. = FALSE)
    }
  )

  stopifnot(inherits(step, "RciStep"))
